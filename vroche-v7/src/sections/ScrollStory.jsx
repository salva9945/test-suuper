...
/* Outfit composition — real PNGs ordered logically as a person would dress.
   Layout strategy: blouse top-center, coat to the right (NOT covering blouse),
   pants center middle, shoes at bottom with safe margin (not cropped),
   bag and accessories around the edges.
   x,y in % (center anchor). size = % of stage width. zIndex for layering. */
const ITEMS = [
  // step 1: top (brown blouse) — center, upper third
  { key: "top",        start: 0.04, img: "/look/top.png",        x: 42, y: 24, size: 26, rot: 0,  z: 5 },
  // step 2: pants (white) — center, just below the top
  { key: "pants",      start: 0.18, img: "/look/pants.png",      x: 42, y: 56, size: 18, rot: 0,  z: 4 },
  // step 3: shoes — bottom area, with enough margin so they don't get cropped
  { key: "shoes",      start: 0.32, img: "/look/shoes.png",      x: 42, y: 82, size: 20, rot: 0,  z: 3 },
  // step 4: coat (beige) — TO THE RIGHT of the top, not covering it
  { key: "coat",       start: 0.46, img: "/look/coat.png",       x: 80, y: 26, size: 28, rot: 4,  z: 5 },
  // step 5: bag (burgundy) — right side mid (below the coat)
  { key: "bag",        start: 0.58, img: "/look/bag.png",        x: 84, y: 64, size: 18, rot: -4, z: 7 },
  // step 6: sunglasses — upper left
  { key: "sunglasses", start: 0.72, img: "/look/sunglasses.png", x: 15, y: 18, size: 20, rot: -10, z: 7 },
  // step 7: ring — small accent, mid-left
  { key: "ring",       start: 0.86, img: "/look/ring.png",       x: 14, y: 56, size: 11, rot: 6,  z: 7 },
];
    ...
    >
      <div className="scroll-story-sticky">
        <div className="mx-auto grid h-full max-w-[1440px] grid-cols-1 gap-0 lg:grid-cols-[1.05fr_1fr] lg:gap-10 px-5 md:px-9">
          ...
          {/* Left: visual stage — vertically centered with safe area */}
